...
st.set_page_config(page_title="IDS/Q8 Fleet Intelligence V3", layout="wide")
...
def build_pdf_buffer(summary_rows, bullet_points, actions, title="Report executive IDS/Q8"):
    buffer = io.BytesIO()
    ...
    story.append(Paragraph("Azioni consigliate", styles["Heading2"]))
    for item in actions:
        story.append(Paragraph(f"• {item}", styles["Normal"]))
        story.append(Spacer(1, 6))
    doc.build(story)
...
st.markdown(
    '<div class="hero-box"><h1 style="margin:0;">🚛 IDS/Q8 Fleet Intelligence V3</h1>'
    '<p style="margin:0.4rem 0 0 0;">Dashboard executive premium: KPI, benchmark, concentrazione, anomalie, insight manageriali e report PDF.</p></div>',
    unsafe_allow_html=True
)

uploaded = st.file_uploader("Carica un file Excel IDS/Q8", type=["xlsx"])
...
month_df = add_period_bucket(valid)
delta_cost = delta_volume = delta_price = None
if month_df is not None and month_df["month"].nunique() >= 2:
    month_agg = month_df.groupby("month").agg(cost=("amount_eur","sum"), volume=("volume","sum"))
    month_agg["avg_price"] = month_agg["cost"] / month_agg["volume"]
    recent = month_agg.iloc[-1]
    prev = month_agg.iloc[-2]
    delta_cost = (recent["cost"] / prev["cost"] - 1) * 100 if prev["cost"] else np.nan
    delta_volume = (recent["volume"] / prev["volume"] - 1) * 100 if prev["volume"] else np.nan
    delta_price = (recent["avg_price"] / prev["avg_price"] - 1) * 100 if prev["avg_price"] else np.nan

st.success(f"File in uso: {Path(raw.attrs.get('source_name', 'n.d.')).name}")

k1, k2, k3, k4 = st.columns(4)
k1.metric("Spesa totale", euro(total_cost), delta=metric_delta_str(delta_cost))
k2.metric("Volume totale", liters(total_volume), delta=metric_delta_str(delta_volume))
k3.metric("Prezzo medio / litro", euro(avg_price), delta=metric_delta_str(delta_price))
k4.metric("Transazioni valide", f"{transactions:,}".replace(",", "."))

conc1, conc2, conc3 = st.columns(3)
if "plate" in valid.columns:
    plate_spend = valid.groupby("plate")["amount_eur"].sum().sort_values(ascending=False)
    conc1.metric("Quota spesa top 5 targhe", percent(plate_spend.head(5).sum() / total_cost if total_cost else np.nan))
if "station" in valid.columns:
    station_spend = valid.groupby("station")["amount_eur"].sum().sort_values(ascending=False)
    conc2.metric("Quota spesa top 5 stazioni", percent(station_spend.head(5).sum() / total_cost if total_cost else np.nan))
conc3.metric("Incidenza righe escluse", percent(len(excluded) / meta["initial_rows"] if meta["initial_rows"] else np.nan))
...
if "station" in valid.columns:
    ...
    if not station_tbl_alert.empty and station_tbl_alert["prezzo_medio_l"].max() - station_tbl_alert["prezzo_medio_l"].min() > 0.15:
...
if "product" in valid.columns:
    ...
    bullets.append(f"Il prodotto principale è {by_product.index[0]} e pesa {percent(by_product.iloc[0] / total_cost)} della spesa totale.")

if "plate" in valid.columns:
    plate_spend = valid.groupby("plate")["amount_eur"].sum().sort_values(ascending=False)
    bullets.append(f"Le top 5 targhe assorbono {percent(plate_spend.head(5).sum() / total_cost)} della spesa.")
    actions.append("Approfondire le targhe con maggiore budget assorbito e verificare frequenza/volumi dei rifornimenti.")
...
if "station" in valid.columns:
    ...
    if not station_tbl.empty:
        ...
        bullets.append(f"La stazione più cara è {expensive.index[0]} ({euro(expensive['prezzo_medio_l'].iloc[0])}/L), la più conveniente è {cheap.index[0]} ({euro(cheap['prezzo_medio_l'].iloc[0])}/L).")
        bullets.append(f"Il saving teorico spostando il 10% dei volumi è circa {euro(saving)}.")
        actions.append("Valutare spostamento volumi verso stazioni più convenienti o aprire confronto commerciale con i punti più cari.")

if len(excluded) > 0:
    bullets.append(f"Sono state escluse {len(excluded)} righe non affidabili dal calcolo dei KPI.")
    actions.append("Ridurre le righe escluse migliorando qualità e completezza dell’export.")

for b in bullets:
    st.markdown(f"- {b}")

st.subheader("📊 Vista manageriale")
row1a, row1b = st.columns(2)
if "product" in valid.columns:
    prod_tbl = (
        valid.groupby("product")
             .agg(transazioni=("product", "size"), volume=("volume", "sum"), spesa=("amount_eur", "sum"))
             .assign(prezzo_medio_l=lambda x: x["spesa"] / x["volume"])
             .assign(quota_spesa=lambda x: x["spesa"] / x["spesa"].sum())
             .sort_values("spesa", ascending=False)
    )
    row1a.plotly_chart(px.bar(prod_tbl.reset_index(), x="product", y="spesa", title="Spesa per prodotto"), use_container_width=True)
    row1b.plotly_chart(px.pie(prod_tbl.reset_index(), names="product", values="spesa", title="Mix spesa per prodotto"), use_container_width=True)
...
if "date" in valid.columns:
    ...
    row2a.plotly_chart(px.line(daily, x="date", y="amount_eur", title="Trend giornaliero spesa"), use_container_width=True)
    daily["cum_amount"] = daily["amount_eur"].cumsum()
    row2b.plotly_chart(px.line(daily, x="date", y="cum_amount", title="Spesa cumulata"), use_container_width=True)

row3a, row3b = st.columns(2)
if "plate" in valid.columns:
    top_plate = valid.groupby("plate")["amount_eur"].sum().sort_values(ascending=False).head(10).reset_index()
    row3a.plotly_chart(px.bar(top_plate, x="plate", y="amount_eur", title="Top 10 targhe per spesa"), use_container_width=True)
if "station" in valid.columns:
    top_station = (
        valid.groupby("station")
             .agg(spesa=("amount_eur", "sum"), volume=("volume", "sum"), transazioni=("station", "size"))
    ...
    if not top_station.empty:
        row3b.plotly_chart(px.bar(top_station, x="station", y="prezzo_medio_l", title="Top 10 stazioni più care €/L"), use_container_width=True)

st.subheader("📌 Tabelle decisive")
tab1, tab2 = st.columns(2)
if "station" in valid.columns:
    station_compare = (
        valid.groupby("station")
             .agg(transazioni=("station", "size"), volume=("volume", "sum"), spesa=("amount_eur", "sum"))
             .query("transazioni >= 3 and volume > 50")
             .assign(prezzo_medio_l=lambda x: x["spesa"] / x["volume"])
             .sort_values("prezzo_medio_l", ascending=False)
    )
    tab1.markdown("**Stazioni più care**")
    tab1.dataframe(station_compare.head(5), use_container_width=True)
    tab2.markdown("**Stazioni più convenienti**")
    tab2.dataframe(station_compare.sort_values("prezzo_medio_l").head(5), use_container_width=True)

st.subheader("⚠️ Anomalie")
anom = valid.copy()
anom["z_price"] = (anom["price_per_liter"] - anom["price_per_liter"].mean()) / anom["price_per_liter"].std(ddof=0)
anom["z_volume"] = (anom["volume"] - anom["volume"].mean()) / anom["volume"].std(ddof=0)
anom["is_anomaly"] = (anom["z_price"].abs() > 2.5) | (anom["z_volume"].abs() > 2.5)
anom_tbl = anom[anom["is_anomaly"]].copy()

an1, an2 = st.columns(2)
an1.metric("Transazioni anomale", f"{len(anom_tbl):,}".replace(",", "."))
an2.metric("Incidenza anomalie", percent(len(anom_tbl) / len(valid) if len(valid) else np.nan))

if not anom_tbl.empty:
    cols = [c for c in ["date", "product", "plate", "station", "volume", "amount_eur", "price_per_liter"] if c in anom_tbl.columns]
    st.dataframe(anom_tbl[cols].sort_values("price_per_liter", ascending=False), use_container_width=True)
    actions.append("Analizzare le transazioni anomale per verificare errori di dato o pattern di rifornimento fuori standard.")
else:
    st.success("Nessuna anomalia rilevante nel perimetro selezionato.")

st.subheader("🧹 Controlli qualità dato")
...
if not actions:
    actions.append("Mantenere monitoraggio periodico dei KPI e benchmark prezzi per stazione.")
...
summary_rows = [
    ...
    ["Transazioni anomale", f"{len(anom_tbl):,}".replace(",", ".")],
]
pdf_buffer = build_pdf_buffer(summary_rows, bullets, list(dict.fromkeys(actions)))
st.download_button(
    "Scarica report PDF",
    data=pdf_buffer,
    file_name=f"report_ids_q8_v3_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf",
    mime="application/pdf"
)

st.caption("Versione V3: layout premium, KPI con confronto, insight manageriali, anomalie e raccomandazioni operative.")
